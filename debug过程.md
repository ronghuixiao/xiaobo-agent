...
**现象**：
Dashboard 今日任务区域显示了 7 个任务，其中 3 个系统任务（早间签到、主动关怀检查、每日日报生成）同时出现在"个人任务"和"系统任务"两个区域。用户只发了 "今日任务：小柏agent完善与整理；数据结构；八股；java项目准备和整理"，不应出现系统任务在个人任务中。

**根因分析**：

系统存在**两套任务创建机制**，它们同时运行且互不感知：

| 机制 | 位置 | 方式 | 保存类型 | 前缀 |
|------|------|------|----------|------|
| ① 正则匹配 | `task_manager.py:detect_task_list` | 解析 "今日任务：A；B；C" | `type="user"` | `user-` |
| ② LLM 提取 | `handler.py:extract_tasks_from_response` | 从 LLM 回复中提取 `[TASKS_DETECTED]` | `type="user"` | `llm-` |

**触发流程**：
1. 用户发 "今日任务：小柏agent完善与整理；数据结构；八股；java项目准备和整理"
2. **机制①** 正确解析出 4 个任务，保存为 `user-` 前缀 ✅
3. 系统提示词把今日所有任务（含 builtin 系统任务）注入给 LLM 作为上下文
4. **LLM 看到系统任务后产生幻觉**，在回复中输出 `[TASKS_DETECTED]- 早间签到\n- 主动关怀检查\n- 每日日报生成[/TASKS_DETECTED]`
5. **机制②** 把这 3 个幻觉任务保存为 `type="user"`（`llm-` 前缀）❌

**线上数据库证据**：
```
user-2026-07-24-423333  | 小柏agent完善与整理 | type=user    | created=15:15:57
user-2026-07-24-600767  | 数据结构           | type=user    | created=15:15:57
user-2026-07-24-481522  | 八股               | type=user    | created=15:15:57
user-2026-07-24-984247  | java项目准备和整理   | type=user    | created=15:15:57
llm-2026-07-24-288226   | 早间签到           | type=user    | created=15:28:46  ← 幻觉
llm-2026-07-24-372855   | 主动关怀检查        | type=user    | created=15:28:46  ← 幻觉
llm-2026-07-24-070085   | 每日日报生成        | type=user    | created=15:28:46  ← 幻觉
today-2026-07-24-0800   | 早间签到           | type=builtin | created=00:00:33
today-2026-07-24-0900   | 主动关怀检查        | type=builtin | created=00:00:33
today-2026-07-24-2200   | 每日日报生成        | type=builtin | created=00:00:33
```

**修复方案（方案A）：完全删除机制②**

`detect_task_list` 已经通过正则正确处理了 "今日任务：..." 格式，LLM 任务提取是冗余且危险的。

**修改内容**：
1. **handler.py**：
   - 删除 `extract_tasks_from_response` 静态方法
   - 删除 `_save_extracted_tasks` 方法
   - 非流式路径：不再调用提取逻辑，直接清理 `[TASKS_DETECTED]` 标签
   - 流式路径：同上
   - 系统提示词：删除 "任务识别（极其严格）" 整个章节（约 30 行），因为不再需要 LLM 输出任务标记

2. **database.py**：新增 `delete_task` 方法（用于清理线上幻觉数据）

3. **routes.py**：新增 DELETE `/api/tasks/{task_id}` 端点（用于清理线上幻觉数据）

**验证**：
- ✅ 系统任务（builtin）不受影响，仍由 `_ensure_builtin_tasks()` 在 daemon 启动/跨天时创建
- ✅ 用户任务（user）仍由 `detect_task_list` 正则匹配正确创建
- ✅ LLM 不再有机会幻觉出假任务
- ✅ 代码语法检查通过
