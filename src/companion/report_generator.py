...
- 如果数据显示"没有情绪记录"或"没有提取新信息"，就如实写"暂无记录"
- 不要编造故事、事件、情绪变化
- 保持简洁，不要超过 500 字
...
class ReportGenerator:
    ...
    async def generate_daily_report(self, target_date: Optional[datetime] = None) -> str:
        """生成日报 - 使用改进版 DailyReportGenerator"""
        from src.companion.daily_report import DailyReportGenerator
        reporter = DailyReportGenerator(self.llm, self.memory)
        return await reporter.generate_daily_report(target_date)
