...
class FeishuConnection:
    ...
    async def send_text(self, chat_id: str, text: str, id_type: str = "chat_id") -> bool:
        """发送文本消息到指定会话
        ...
        try:
            async with self._session.post(
                f"{FEISHU_BASE_URL}/im/v1/messages?receive_id_type={id_type}",
                json=payload,
    ...
    async def broadcast(self, chat_id: str, text: str) -> bool:
        ...
        默认使用 chat_id 类型
        """
        return await self.send_text(chat_id, text, id_type="chat_id")
